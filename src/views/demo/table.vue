        ...
        >
          <template #header>
            <div class="table-head">
              ...
              <el-dropdown trigger="click" @command="(command: string) => handleCommand(command, oneCol)">
                <i-ep-arrow-down class="action-more"></i-ep-arrow-down>
                <template #dropdown>
                  <el-dropdown-menu>
                    <template v-if="oneCol.prop === 'id'">
                      <el-dropdown-item :command="TableCommand.ASC"> 升序 </el-dropdown-item>
                      <el-dropdown-item :command="TableCommand.DESC"> 降序 </el-dropdown-item>
                    </template>
...
const currentSheetName = computed(() => {
  const find = state.sheetList.find(item => item.sheetId === state.activeSheet)
  return find?.name || ''
})
...
const readFile = (file: any, callback: any) => {
  ...
  reader.onload = function (e) {
    ...
    const result = XLSX.utils.sheet_to_json(state.allSheetObj[currentSheetName.value], {
      raw: false,
      dateNF: 'YYYY-MM-DD HH:mm:ss'
    })
    const allData = XLSX.utils.sheet_to_json(state.allSheetObj[currentSheetName.value], { header: 1 })
